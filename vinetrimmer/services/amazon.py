            ...
            for season in seasons:
                ...
                try:
                    cards = [
                        ...
                        for x in data["episodeList"]["episodes"]]
                except:
                    pass
...
#                            silent=True
    ...
    def pr_license(self, challenge: bytes,track, title: Title, **_):
        try:
            lic = self.session.post(
                ...
                params={
                    ...
                    "deviceStreamingTechnologyOverride": "SmoothStreaming" if track.smooth else "DASH",
                    "deviceVideoQualityOverride": "UHD" if self.quality == 2160 else "HD",
                ...
                data={
                    "playReadyChallenge": base64.b64encode(challenge).decode("utf-8"),  # expects base64
                }
            ).json()
        except:
            lic = self.session.post(
    ...
    def license(self, challenge: bytes,track, title: Title, **_):
        
        if self.cdm.device.type == LocalDevice.Types.PLAYREADY:
            license_res=self.pr_license(challenge,track,title)
            return license_res
        lic = self.session.post(
            url=self.endpoints["licence"],
            params={
    ...
    def configure(self) -> None:
        ...
        if (self.quality > 1080 or self.range != "SDR") and self.vcodec == "H265" and self.cdm.device.type in [LocalDevice.Types.CHROME, LocalDevice.Types.PLAYREADY]:
            self.log.info(f"Using device to get UHD manifests")
            self.register_device()
        elif not self.device or self.cdm.device.type in [LocalDevice.Types.CHROME, LocalDevice.Types.PLAYREADY] or self.vquality != "UHD":
    ...
    def get_best_quality(self, title):
        ...
        for bitrate in bitrates:
            ...
            if self.event:
                devicetype = self.device["device_type"]
                mpd_url = chosen_manifest["avUrlInfoList"][0]["url"]
